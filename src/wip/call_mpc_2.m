function [Y, U] = call_mpc_2( x0, p0, p1 , A, B, G, N)
...
%      end
    ...
    minimize( Cg*norm(Y(xyz_loc,end),'fro') + Cp*norm(U,'fro' ))%Cp*norm(S-Y(xyz_loc,:),'fro') + )% + norm( Rsqrt*U, 'fro') ) %
    subject to
        %dynamics
        Y(:,1) == y0;
